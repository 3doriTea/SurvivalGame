...
namespace GameBase::System
{
	...
	struct IEditorGui : ISystemInterfaceBase
	{
		...
		virtual void OnGUI(const std::function<void(EventSubject<EntityRegistry&>&)>& _callback) = 0;
	};
	...
	public:
		...
		void OnGUI(const std::function<void(EventSubject<EntityRegistry&>&)>& _callback) override;
	...
	private:
		...
		EventSubject<EntityRegistry&> onGUIEvent_;
	};
}
